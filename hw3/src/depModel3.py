...
class DepModel:
    def __init__(self):
        ...
        # use pickle to load vocab and network properties.
        self.vocab, self.net_properties = pickle.load(open("./data/pkl3-3", 'r'))
        # construct the default network
        self.network = Network(self.vocab, self.net_properties)
        # load the network trained model1
        self.network.load("./data/model3-3")
    ...
    def score(self, str_features):
        ...
        result = self.network.build_graph(str_features).value()
        dynet.renew_cg()
        return result
